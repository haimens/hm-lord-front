      ...
      <div style={styles.shadow} className="bg-white shadow-sm">
          ...
          <section className="row p-3 border-bottom">
            <div className="d-flex col-12  flex-sm-row flex-column mb-sm-0 justify-content-sm-between  mb-5 ">
              <div className="d-flex flex-row m-0 col-12 p-0 col-sm-9">
                <div className="mr-3">
                  本日总订单 : <span>{this.props.daily_order_data.total_count}</span> {applyCurrencySign(" 笔")}
                </div>
                <div className="ml-3">
                  与昨日相比 :{" "}
                  {this.readCompare(this.props.daily_order_data.total_count, this.props.past_data.daily_total_count)}
                </div>
              </div>
              <div className="d-flex input-group col-sm-3 col-12 p-0 ">
                <input
                  ...
                  value={this.state.dailyTab.currentDate}
            ...
            <div className="col-md-12 col-sm-12 d-flex justify-content-between ">
              <div>
                <Button
                  label="全部"
                  className={this.state.activeTab === 0 ? "hm-bg-green text-white" : "mr-button-purple mr-purple-text"}
                  onClick={() => this.daySwitch(0)}
                />
                <Button
                  label="微信支付"
                  className={this.state.activeTab === 1 ? "hm-bg-green text-white" : "mr-button-purple mr-purple-text"}
                  onClick={() => this.daySwitch(1)}
                />
                <Button
                  label="支付宝"
                  className={this.state.activeTab === 2 ? "hm-bg-green text-white" : "mr-button-purple mr-purple-text"}
                  onClick={() => this.daySwitch(2)}
                />
                <Button
                  label="QQ钱包"
                  className={this.state.activeTab === 3 ? "hm-bg-green text-white" : "mr-button-purple mr-purple-text"}
                  onClick={() => this.daySwitch(3)}
                />
              </div>
            </div>
          </section>
        ...
        <section className="row p-1">
          <div className="col-12">
            <LineChart
              notAmount={true}
              dailyData={this.props.daily_order_data}
              xLabel={"时间（小时）"}
              yLabel={`订单数 (${applyCurrencySign()})`}
            />
          </div>
        </section>
      </div>
